...
@Service
public class CreateTransferAccountJob extends SpringApplicationObjectSupport{
    @Autowired
    private ITxnsLogService txnsLogService;
    
    private static final Log log = LogFactory.getLog(SaveMemberQueueJob.class);
        
    private static final String FILE_PREX = "TRANSFER";
...
//        String datetimeString ="20160915";
        
        // 根据日期取出t_txns_log取出当天所有的转账商户受理清算日期、受理二级商户号       
        List<?> memberList = txnsLogService.geTransferAccountMemberByDate(dateTime);
        ...
        for (Map.Entry<String, String> entry : memIDMap.entrySet()){
            ...
            // 转账汇总信息
            List<?> sumTransferAccountList = txnsLogService.getSumTransferAccount(memberId,
                    dateTime);
            ...
            // 总手续费
            Long sumFee = 0L;
            if (sumTransferAccountList != null && sumTransferAccountList.size() > 0) {
                JSONArray json = JSONArray.fromObject(sumTransferAccountList);
            ...
            List<?> transferDetailList = txnsLogService.getTransferAccountByDate(memberId,dateTime);
            JSONArray detailJsonArray = JSONArray
                    .fromObject(transferDetailList);

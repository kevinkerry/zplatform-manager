...
@Service
public class CreateBondSDFileJob extends SpringApplicationObjectSupport{
    @Autowired
    ...
    public void execute() throws Exception{
        ...
        List<?> memberList = txnsLogService.getBondMemberByDate(dateTime);
        ...
        for (Map.Entry<String, String> entry : memIDMap.entrySet()){
            ...
            // 保证金汇总信息
            List<?> sumBondList = txnsLogService.getSumBond(memberId,
                    dateTime);
            ...
            List<?> bondList = txnsLogService.getBondByDate(memberId,dateTime);
            ...
            for (int i = 0; i < detailJsonArray.size(); i++) {
                ...
                fileBuffer.append(job.get("ACCORDNO").toString())
                        ...
                        .append(DELETIMER).append(flag).append(DELETIMER)

...
//对应页面功能模块：风控管理；银行卡黑名单，银行卡白名单,商户黑名单，分卡种单笔限额
	...
	public String  queryRisk(){
		...
		Map<String, Object> groupList=serviceContainer.getRiskService().findRiskByPage(variables, getPage(), getRows());
	...
	public String  queryOneRisk(){
		Map<String, Object> feeList=serviceContainer.getRiskService().queryOneRisk(riskId);
		json_encode(feeList);
		return null;
	}
	public String  queryOneRiskCase(){
		Map<String, Object> feeList=serviceContainer.getRiskService().queryOneRiskCase(riskId);
		json_encode(feeList);
		return null;
	}
	public String  UpdateOneRisk(){
		...
		riskModel.setInuser(getCurrentUser().getUserId());
		String mark=serviceContainer.getRiskService().UpdateOneRisk(riskModel);
		json_encode(mark);
		return null;
	}
	...
	public String queryRiskCase(){
		List<?> riskCaseList=serviceContainer.getRiskService().queryRiskCase(riskver);
		try {
	...
	public String queryRisklistCheck(){
		List<?> riskcheckList=serviceContainer.getRiskService().query_risk_list();
		try {
	...
	public String queryRisklistCheckActive(){
		List<?> riskcheckList=serviceContainer.getRiskService().query_risk_list_active(riskId);
		try {
				json_encode(riskcheckList);
			} catch (IOException e) {
				e.printStackTrace();
	...
	public String  updateRiskCase(){
		...
		String mark=serviceContainer.getRiskService().UpdateOneRiskCase(riskCaseModel);
		json_encode(mark);
		return null;
	}
	//风控版本
	public String queryRisklist(){
			List<?> riskcheckList=serviceContainer.getRiskService().query_risk_all();
			try {
					json_encode(riskcheckList);
				} catch (IOException e) {
					e.printStackTrace();
	...
	public String queryRisklistCase(){
			List<?> riskcheckList=serviceContainer.getRiskService().query_risk_case_all(riskId);
			try {
					json_encode(riskcheckList);
				} catch (IOException e) {
					e.printStackTrace();
	...
	public String  queryBlackPanByPage(){
		...
		Map<String, Object> groupList=serviceContainer.getRiskService().findBlackPanByPage(variables, getPage(), getRows());
		json_encode(groupList);
		return null;
	}
	//风险等级
	...
	//-------------------------------------------------- 银行卡白名单------------------------------------------------------------------------
		...
		public String  queryWhitePanByPage(){
			...
			Map<String, Object> groupList=serviceContainer.getRiskService().findWhitePanByPage(variables, getPage(), getRows());
			json_encode(groupList);
			return null;
		}
...
//--------------------------------------------------------------卡类别日累计限额------------------------------------------------------
		...
		public String  queryBlacklistMemberByPage(){
			...
			Map<String, Object> LimitMemList=serviceContainer.getRiskService().findBlacklistMemberByPage(variables, getPage(), getRows());
...
//--------------------------------------------------------------分卡种单笔限额------------------------------------------------------
		...
		public String  queryLimitSingleByPage(){
			...
			Map<String, Object> LimitMemList=serviceContainer.getRiskService().findlimitCreditSingleByPage(variables, getPage(), getRows());
			json_encode(LimitMemList);
			return null;
		}
		...
		public String deleteLimitCreditSingle(){
			String mark=serviceContainer.getRiskService().deleteOnelimitCreditSingle(riskId,getCurrentUser().getUserId());
			json_encode(mark);
			return null;
		}
	...
	public void setBlacklistMemberModel(BlacklistMemberModel blacklistMemberModel) {

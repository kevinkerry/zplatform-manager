...
public class CreateMemberFileJob {
    ...
    public void execute() throws Exception {
      ...
      List<?> memberListA=  txnsLogService.getAllMemberByDateByCharge(dateTime);
      ...
      for (Map.Entry<String, String> entry : memIDmap.entrySet()) {
          ...
          //提现,退款,代付账户资金减少
          List<?> countReList =txnsLogService.getCountRefundAndPay(memberId,dateTime);
          //消费-账户 账户资金增加
          List<?> spendingList =txnsLogService.getCountSpendingAccount(memberId,dateTime);
          memberId="200000000000593";
          //手工充值 账户资金增加
          List<?> handPayList =txnsLogService.getCountHandPay(memberId,dateTime);
          //总交易笔数
          int count=0;
          //总清算金额
          ...
          if(countList!=null&&countList.size()>0){
              ...
              count+=Integer.parseInt(json.getJSONObject(0).get("TOTAL").equals("null")?"0":json.getJSONObject(0).get("TOTAL").toString());
              countClearMoney+=Long.parseLong(json.getJSONObject(0).get("CLEARING").equals("null")?"0":json.getJSONObject(0).get("CLEARING").toString());
          ...
          //[{TXNFEE=50, PAYORDNO=2016011200000088, AMOUNT=261, BUSICODE=10000001}, {TXNFEE=50, PAYORDNO=2016011200000090, AMOUNT=245, BUSICODE=10000001}, {TXNFEE=50, PAYORDNO=2016011200000087, AMOUNT=250, BUSICODE=10000001}, {TXNFEE=50, PAYORDNO=2016011200000089, AMOUNT=247, BUSICODE=10000001}, {TXNFEE=50, PAYORDNO=2016011200000075, AMOUNT=251, BUSICODE=10000001}, {TXNFEE=50, PAYORDNO=2016011200000086, AMOUNT=251, BUSICODE=10000001}, {TXNFEE=50, PAYORDNO=2016011200000083, AMOUNT=251, BUSICODE=10000001}, {TXNFEE=50, PAYORDNO=2016011200000079, AMOUNT=2501, BUSICODE=10000001}, {TXNFEE=50, PAYORDNO=2016011200000084, AMOUNT=251, BUSICODE=10000001}, {TXNFEE=50, PAYORDNO=2016011200000085, AMOUNT=251, BUSICODE=10000001}, {TXNFEE=50, PAYORDNO=2016011200000082, AMOUNT=251, BUSICODE=10000001}]
          ...
          fileBuffer.append("\n");
          fileBuffer.append("######");
          doPrint(fileBuffer.toString(),"D:\\AAA\\"+memberId+"\\",memberId+"_"+dateTime+".txt",memberId);
         }
    }

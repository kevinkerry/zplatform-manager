...
public class TransferAction extends BaseAction {
    ...
    public void queryDetail() {
        if (queryTransferBean != null) {
            if (StringUtil.isNotEmpty(queryTransferBean.getBatchNo())) {
                Map<String, Object> map = transferService.queryDetaTransfer(
                        queryTransferBean, getPage(), getPage_size());
                json_encode(map);
            }
        }
    ...
    public void batchTrail() {
        ...
        for (String batchno : batchId_array) {
            transferService.transferBatchTrial(Long.parseLong(batchno.trim()),
                    auditBean.getFalg());
        }
    ...
    public void trailTransferDeta() {
        ...
        for (String orderNo : batchno_array) {
            transferService.transferDataTrial(Long.valueOf(orderNo),
                    auditBean.getFalg());
        }
    }
    ...
    public void queryBankData() {
        if (queryTransferBean != null) {
            if (StringUtil.isNotEmpty(queryTransferBean.getBatchNo())) {
                Map<String, Object> map = bankTransferService
                        .queryDataBankTransfer(queryTransferBean, getPage(),
                                getPage_size());
                json_encode(map);
            }
        }
    ...
    public void batchBankTrial() {
        ...
        for (String batchno : batchno_array) {
            bankTransferService.bankTransferBatchTrial(batchno.trim(),
                    auditBean.getFalg());
        }
    }

...
//       String dateTime = "20160715";
    ...
    public void execute1() throws Exception {
        ...
        String dateTime = sdf.format(date);
        // 根据日期取出t_txns_log取出当天清算的所有商户号
        List<?> memberList = txnsLogService.getSinglepaymentMemberByDate(dateTime);
        ...
        for (Map.Entry<String, String> entry : memIDMap.entrySet()) {
           ...
           /* List<?> sumInsteadList = txnsLogService.getSumInstead(memberId,
                    dateTime);*/
            List<?> sumSinglePaymentList = txnsLogService.getSumSinglepayment(memberId,dateTime);
           ...
           /* List<?> memberDetailedList = txnsLogService
                    .getInsteadMerchantDetailedByDate(memberId, dateTime);*/
            List<?> memberDetailedList = txnsLogService.getSinglepaymentDetailByDate(memberId,dateTime);
            JSONArray detailJsonArray = JSONArray
                    .fromObject(memberDetailedList);
            for (int i = 0; i < detailJsonArray.size(); i++) {
                long amount = 0;
            ...
            fileBuffer.append("######");
            String filePath = RECON_FILE_LOCAL_ROOT_DIR+"/" + memberId + "/";
            String fileName = FILE_PREX1 + "_" + memberId + "_" + dateTime
                    + ".txt"; 
             doPrint(fileBuffer.toString(), filePath, fileName, memberId);
        }
    }
...
//        String dateTime ="20161018";
        // 根据日期取出t_txns_log取出当天清算的所有商户号
        List<?> memberList = txnsLogService.getSingleInsteadMemberByDate(dateTime);
        ...
        for (int i = 0; i < jsonArray.size(); i++) {
            job = jsonArray.getJSONObject(i);
            memIDMap.put(job.get("MER_ID").toString(),
                    job.get("MER_ID").toString());
        }
        ...
        for (Map.Entry<String, String> entry : memIDMap.entrySet()) {
            ...
            fileBuffer.append(DELETIMER).append(DATE).append(":" + dateTime);
            // 代付汇总信息
            List<?> sumInsteadList = txnsLogService.getSumSingleInstead(memberId,dateTime);
            ...
            List<?> memberDetailedList = txnsLogService
                    .getSingleInsteadDetailedByDate(memberId, dateTime);
            JSONArray detailJsonArray = JSONArray
                    .fromObject(memberDetailedList);
            for (int i = 0; i < detailJsonArray.size(); i++) {
                long amount = 0;
                ...
                fileBuffer.append("\n");
                fileBuffer.append(job.get("ORDERNO").toString())
                        .append(DELETIMER)
                        .append(job.get("TXNSEQNO").toString())
                        .append(DELETIMER).append(job.get("ORDER_COMMI_TIME"))
                        .append(DELETIMER).append("70000002")
                        .append(DELETIMER).append("D")
                        .append(DELETIMER).append(amount)
                        .append(DELETIMER).append(job.get("RESP_CODE"))
                        .append(DELETIMER).append(job.get("RESP_MSG"))
                        .append(DELETIMER).append(job.get("RETURN_DATE"));
...
//        String dateTime ="20161018";
        // 根据日期取出t_txns_log取出当天清算的所有商户号
        List<?> memberList = txnsLogService.getBatchpaymentMemberByDate(dateTime);
        ...
        for (Map.Entry<String, String> entry : memIDMap.entrySet()) {
            ...
            fileBuffer.append(DELETIMER).append(DATE).append(":" + dateTime);
            // 代付汇总信息
            List<?> sumInsteadList = txnsLogService.getSumBatchpayment(memberId,dateTime);
            ...
            List<?> memberDetailedList = txnsLogService
                    .getBatchpaymentDetailedByDate(memberId, dateTime);
            JSONArray detailJsonArray = JSONArray
                    .fromObject(memberDetailedList);
            for (int i = 0; i < detailJsonArray.size(); i++) {
                long amount = 0;
                ...
                fileBuffer.append("\n");
                fileBuffer.append(job.get("ORDER_ID").toString())
                        .append(DELETIMER)
                        .append(job.get("TXNSEQNO").toString())
                        .append(DELETIMER).append(job.get("TXN_TIME"))
                        .append(DELETIMER).append("70000001")
                        .append(DELETIMER).append("D")
                        .append(DELETIMER).append(amount)
                        .append(DELETIMER).append(job.get("RESP_CODE"))
                        .append(DELETIMER).append(job.get("RESP_MSG"))
                        .append(DELETIMER).append(job.get("RETUR_NDATE"));

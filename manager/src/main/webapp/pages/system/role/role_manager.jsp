			...
			<div region="center" border="false" style="padding:10px;background:#fff;border:1px solid #ccc;text-align: center">
				...
				<table width="100%" cellpadding="2" cellspacing="2" style="text-align: left">
					...
					<tr>
						<td>备注</td>
						<td colspan="3">
							<textarea rows="3" cols="60" id="role_notes"  maxlength="50" name="role.notes"></textarea>
						</td>
					</tr>
  	...
  	var width = $("#continer").width();
		$(function(){
			...
			$('#test').datagrid({
				...
				nowrap: false,
				striped: true,
				url:'pages/system/queryRoleAction.action',
				singleSelect:true,
				remoteSort: false,
				idField:'ROLE_ID',
				columns:[
		...
		function showAdd(){	
			...
			$('#roleForm :input').val('');
			$("#roleForm").attr("action","pages/system/saveRoleAction.action");
		...
		function search(){
			var data={'role.roleName':$('#roleName').val(),"role.deptId":$("#roleDeptId").val(),'role.organId':$("#roleOrganId").val()};
		...
		function showRole(roleId){		
			...
			$("#roleForm").attr("action","pages/system/updateRoleAction.action");
			$('#btn_submit').linkbutton('enable');	
		}
		...
		function showOrgan(){
			...
			$.ajax({
				type: "GET",
			  	url: "pages/system/showOrganRoleAction.action",

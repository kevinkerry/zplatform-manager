	...
	<div id="w" class="easyui-window" closed="true" title="My Window" iconCls="icon-save" style="width:500px;height:200px;padding:5px;">
		<div class="easyui-layout" fit="true">
			<div region="center" border="false" style="padding:10px;background:#fff;border:1px solid #ccc;text-align: center">
				<form id="txnRateForm" action="pages/fee/saveTxnRateFeeAction.action" method="post" >
				<table width="100%" cellpadding="2" cellspacing="2" style="text-align: left">
					<tr>
						...
						<td width="15%" >业务</td>
						<td>
							<select id="busicase" class="easyui-validatebox"  required="true" name="busiRateModel.busicode" >
							<option value="">--请选择业务--</option>
							</select>
						</td>
					</tr>
					<tr>
						<td>扣率类型</td>
						<td>
							<select id="feeRateType" class="easyui-validatebox"  required="true" name="busiRateModel.rateType" onchange="showRateDetail()">
						...
						<td>
							<input id="feeRate" name="busiRateModel.feeRateStr" validType="percent"   required="true" type="text" class="easyui-validatebox" maxlength="5"/>
						</td>
					</tr>
					<tr>
						<td>最低收费额(单位:元)</td>
						<td>
							<input id="minFee" name="busiRateModel.minFeeStr" maxlength="12" validType="amount" required="true" type="text"  class="easyui-validatebox"  />
						</td>
						<td width="15%">最高收费额(单位:元)</td>
						<td>
							<input id="maxFee" name="busiRateModel.maxFeeStr"  required="true" maxlength="12" validType="amount" type="text"  class="easyui-validatebox" />
						</td>
					</tr>
					<tr>						
						<td>备注</td>
						<td colspan="3">
							<textarea id="notes" rows="3" cols="75" name="busiRateModel.notes" maxlength="32"></textarea>
						</td>
					</tr>
				</table>
				</form>
	  ...
	  $(function() {
		...
		$(function() {
			$('#test').datagrid({
				...
				nowrap: false,
				striped: true,
				url: 'pages/fee/queryTxnRateFeeAction.action',
				remoteSort: false,
				columns: [[{
					field: 'FEENAME',
					title: '扣率版本',
		...
		function search() {
			var data = {
				'busiRateModel.feever': $('#busiPack_ins').val(),
				'busiRateModel.busicode': $("#busiCase_ins").val()
			};

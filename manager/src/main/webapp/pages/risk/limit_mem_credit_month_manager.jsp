...
<html>
	...
	<body>
		<div style="margin: 5px; border: " id="continer">
			<div id="p" class="easyui-panel" title="商户分卡种月限次限额查询"
					...
					<table width="100%">
						<tr>
							...
							<td align="left" style="padding-left: 5px" width="25%">
							  <select id="cardtype_qid" class="easyui-validatebox"   missingMessage="请选择卡类型"  >
		...
		<div class="easyui-layout" fit="true">
			<div region="center" border="false" style="padding:10px;background:#fff;border:1px solid #ccc;text-align: center">
				...
				<input name="limitMemCreditMonthModel.TId" id="TId" type="hidden"/>
				<table width="100%" cellpadding="2" cellspacing="2" style="text-align: left" id="inputForm">
					<tr>
						<td align="right" width="15%" height="30px" >会员号</td>
						<td align="left" style="padding-left:5px" width="25%">
							<input name="limitMemCreditMonthModel.memberid" id="memberid" validType="merchno" maxlength="15" class="easyui-validatebox" />
						</td>
						<td align="right" width="15%">卡类别</td>
						<td align="left" style="padding-left: 5px" width="25%">
							<select id="cardtype" class="easyui-validatebox"  required="true"  missingMessage="请选择卡类型"  name="limitMemCreditMonthModel.cardType" >
							<option value="" selected="selected">--请选择卡类型--</option>
							<option value="1">--借记--</option>
							<option value="2">--贷记--</option>
							<option value="3">--准贷记--</option>
					...
					<tr>
						...
						<td align="left" style="padding-left:5px" width="25%">
							<input name="limitMemCreditMonthModel.limitAmount" id="limitAmount"  validType="amount"   maxlength="11" class="easyui-validatebox" />
						</td>
						<td align="right" width="15%">累计限次</td>
						<td align="left" style="padding-left: 5px" width="25%">
							<input name="limitMemCreditMonthModel.limitCount" id="limitCount" maxlength="8" onkeyup="this.value=this.value.replace(/[^\d]/g,'') "  />
						</td>
					</tr>
					<tr>
					    <td align="right" width="15%" height="30px">风险等级</td>
						<td align="left" style="padding-left: 5px" width="25%">
							<select id="risklevel" class="easyui-validatebox" missingMessage="请选选择风险等级" required="true" name="limitMemCreditMonthModel.risklevel" class="easyui-validatebox">
								<option value="">--请选择风险等级--</option>
							</select>
						</td>
						<td align="right" width="15%">备注</td>
						<td align="left" style="padding-left: 5px" width="25%">
							<input name="limitMemCreditMonthModel.notes" id="Notes" maxlength="32"/>
						</td>
					</tr>
				</table>
				</form>
	...
	<script>
		...
		$(function() {
			$('#test').datagrid({
				...
				striped: true,
				sortName: 'FEEVER',
				url: 'pages/risk/queryLimitMemCreditMonthByPageLimitAction.action',
				remoteSort: false,
				columns: [[{
					field: 'MEMBERID',
					title: '会员号',
		...
		function search() {
			var data = {
				'limitMemCreditMonthModel.memberid': $('#merber_qid').val(),
				'limitMemCreditMonthModel.cardType': $('#cardtype_qid').val()
			};
		...
		function showAdd() {
			...
			$("#theForm").attr("action", "pages/risk/saveLimitMemCreditMonthLimitAction.action");
		...
		function showLimitMem(tid) {
			$.ajax({
				type: "POST",
				url: "pages/risk/queryLimitMemCreditMonthLimitAction.action",
				data: "riskId=" + tid,
				dataType: "json",
				success: function(json) {
					$("#TId").val(json.T_ID);
			...
			$("#theForm").attr("action", "pages/risk/updateLimitMemCreditMonthLimitAction.action");
			$('#btn_submit').linkbutton('enable');
		}
		function deleteLimitMem(tid) {
			$.messager.confirm('提示', '您是否想要注销此商户分卡种月限次限额?',
			function(r) {
				if (r) {
					$.ajax({
						type: "GET",
						url: "pages/risk/deleteLimitMemCreditMonthLimitAction.action",
		...
		function startLimitMem(tid) {
			$.messager.confirm('提示', '您是否想要启用此商户分卡种月限次限额?',
			function(r) {
				if (r) {
					$.ajax({
						type: "GET",
						url: "pages/risk/startLimitMemCreditMonthLimitAction.action",
						data: "rand=" + new Date().getTime() + "&riskId=" + tid,
						dataType: "text",
						success: function(text) {
							$.messager.alert('提示', text);
